...
func TestBetaJigUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Beta.Jig.Update(
		...
		together.BetaJigUpdateParams{
			...
			Memory:                        together.Float(1000),
			MinReplicas:                   together.Int(0),
			Name:                          together.String("x"),
			Port:                          together.Int(1),
			Storage:                       together.Int(400),
...
func TestBetaJigList(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Beta.Jig.List(context.TODO())
	if err != nil {
		var apierr *together.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
